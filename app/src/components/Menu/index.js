...
function ProfessorSearch({ history }) {
    ...
    const onSubmit = (data) => {
        if (data.key === 'Enter' && isValid) {
            history.push("/" + whoAuthenticated() + "/search/" + data.target.value.trim())
        }
    }
    const validate = (e) => {
    ...
    function makeMenuOptions() {
        if (menuOptions === "") {
            return (setMenuOptions(
                <MenuOptions>
                    <Button type='button' backColor='#FFD54F' text='CONFIGURAR' padding='3px' onClick={() => {setMenuOptions(""); history.push('/student/profile')}}/>
