...
describe('Snapshot Student Profile view', () => {
  it('Testing the useEffect of Header', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('Nome Completo: Estudante Teste')).toBeInTheDocument()
      expect(screen.getByText('E-mail: 123456789@aluno.unb.br')).toBeInTheDocument()
      expect(screen.getByText('Curso: Engenharia de Software')).toBeInTheDocument()
    })
  });
  ...
  it('Testing the useEffect of Body', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('Quantidade de avaliações realizadas: 1')).toBeInTheDocument()
      expect(screen.getByText('Quantidade de pessoas que concordaram com suas avaliações: 2')).toBeInTheDocument()
      expect(screen.getByText('Quantidade de pessoas que discordaram de suas avaliações: 1')).toBeInTheDocument()
