...
describe('Snapshot ProfessorHome view', () => {
  it('Assyncronous test for useEffect', async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText(
          `[${validPost.discipline.discipline_code}] ${validPost.discipline.name}`,
        ),
      ).toBeInTheDocument();
    });
    await waitFor(() => {
  ...
  it('Clicking on buttons and expecting changes', async () => {
    ...
    fireEvent.click(screen.getByText('ESTATÍSTICA'));

    await waitFor(() => {
      expect(screen.getByText('Estatística')).toBeTruthy();
    });
