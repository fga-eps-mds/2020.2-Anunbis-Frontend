...
describe('Snapshot Auth component', () => {
  it('Write user student on localStorage', async () => {
    ...
    expect(window.localStorage.getItem('student')).toEqual(
      JSON.stringify(validStudent.response),
    );
  });
  it('Write user professor on localStorage', async () => {
    ...
    expect(window.localStorage.getItem('professor')).toEqual(
      JSON.stringify(validProfessor.response),
    );
  });
